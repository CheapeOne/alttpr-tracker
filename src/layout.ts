...
import { dungeons, map } from './trackables.ts';
...
export const settings = {
  ...
  applyCookie: function () {
    ...
    const savedSettings = typeof settings.getCookie('settings') !== 'undefined'
      ? JSON.parse(settings.getCookie('settings'))
      : {};
  ...
  apply: function (reset) {
    ...
    $.each(layout.positions[settings.layout].all, function (k, v) {
      if (typeof (v.props !== 'undefined')) {
        TweenMax.set(v.elem, v.props);
      }
      if (settings.border == 0) {
        TweenMax.set(v.elem, v.borderless);
      }
    });
...
export const layout = {
  positions: {
    ...
    0: {
      //Square-ish Layout. Relatively Compact. Default
      all: [
    ...
    1: {
      //Arranges dungeons by their map positions; a little less economical
      all: [
        { elem: '.icon,.tracker', props: { x: 0, y: 0 } }, //resets all icons
        { elem: '#gear', props: { width: 176, height: 312 }, borderless: { x: '-=32', y: '-=4' } },
        { elem: '#items', props: { width: 372, height: 312 }, borderless: { x: '-=12', y: '-=0' } },
    ...
    3: {
      ...
      0: [
        ...
        {
          elem: '#dungeons',
          props: { x: 556, y: -316, width: 352, height: 312 },
          borderless: { x: '-=100', y: '-=0' },
        },
      ...
      1: [
        ...
        {
          elem: '#dungeons',
          props: { x: 556, y: -316, width: 500, height: 312 },
          borderless: { x: '-=100', y: '-=0' },
        },
        { elem: '#dungeonChest11,.key,.bigKey', props: { display: 'inline' } },
