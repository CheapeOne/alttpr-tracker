...
stats = {
  ...
  goList8: function () {
    ...
    list = [
      ...
      (medallion == 3 || (medallion == 0 && (items.medal8.val == 1 || items.medal8.val == 0))) && !items.bombos.val
        ? 'bombos'
        : '',
      (medallion == 3 || (medallion == 0 && (items.medal8.val == 2 || items.medal8.val == 0))) && !items.ether.val
        ? 'ether'
        : '',
      (medallion == 3 || (medallion == 0 && (items.medal8.val == 3 || items.medal8.val == 0))) && !items.quake.val
        ? 'quake'
        : '',
    ];
  ...
  goList9: function () {
    ...
    list = [
      ...
      (medallion == 3 || (medallion == 0 && (items.medal9.val == 1 || items.medal9.val == 0))) && !items.bombos.val
        ? 'bombos'
        : '',
      (medallion == 3 || (medallion == 0 && (items.medal9.val == 2 || items.medal9.val == 0))) && !items.ether.val
        ? 'ether'
        : '',
      (medallion == 3 || (medallion == 0 && (items.medal9.val == 3 || items.medal9.val == 0))) && !items.quake.val
        ? 'quake'
        : '',
    ];
  ...
  find: function (elem) {
    ...
    if (settings.predictor !== '0') {
      if (
        ...
        elem.id == 'bomb' ||
        elem.id.indexOf('ey') >= 0 ||
        items[elem.id].val == items[elem.id].max
      ) {
        item = null;
      ...
      if (item !== null && typeof item[0] !== 'undefined' && typeof statsTable[item[0]] !== 'undefined') {
        ...
        $.each(item, function (k, itemName) {
          $.each(statsTable[itemName], function (label, count) {
            if (typeof counts[label] == 'undefined') {
              counts[label] = 0;
            }
        ...
        $.each(dungeons, function (id, dungeon) {
          ...
          if (typeof numChests !== 'undefined' && unopened !== 0 && (access || settings.predictor == 2)) {
            ...
            if (id !== '10') {
              //ganon's tower has so many chests it unbalances the colour scale, so it is not factored in
              total += count;
              max = Math.max(max, count);
            }
