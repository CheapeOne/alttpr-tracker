...
export const logic = {
  ...
  darkWorldSouth: function () {
    //check for access to this whole region
    return (
      logic.darkWorldNW() || //can drop down from village -- includes hammer + glove option
      (items.boss11.val && items.pearl.val && items.hammer.val)
    ); //agahnim + hammer also works
  },
  DMlight: function () {
    return items.lamp.val || items.flute.val;
  }, //used to determine if DM chests require dark
  climbDM: function () {
  ...
  cane: function () {
    return items.somaria.val || items.byrna.val;
  }, //the canes work against certain bosses
  rod: function () {
    return items.firerod.val || items.icerod.val;
  }, //the rods work against certain bosses
  fire: function () {
    return items.lamp.val || items.firerod.val;
  }, //can light torches
  //Dungeon entry
  entry0: function () {
    return true;
  ...
  entry4: function () {
    return logic.darkWorldSouth() && items.mirror.val && items.flippers.val;
  },
  entry5: function () {
  ...
  entry7: function () {
    return (
      ...
      items.glove.val >= 2 &&
      items.flippers.val &&
      (items.firerod.val || (items.bombos.val && items.sword.val >= 1))
    );
  },
  entry8: function () {
    return (
  ...
  entry11: function () {
    return items.sword.val >= 2 || items.cape.val;
  },
  ...
  setPrizes: function () {
    ...
    if (items.pendant.val >= 3) {
      //if you have three pendants, 1 must be green
      items.greenPendant.val = 1;
    }

    if (items.crystal.val >= 7) {
      //if you have 7 crystals, 2 must be red
      items.redCrystal.val = Math.max(2, items.redCrystal.val);
    } else if (items.crystal.val >= 6) {
      //if you have 6 crystals, at least 1 is red
  ...
  chests: {
    ...
    6: function () {
      ...
      return items.pendant.val == 3 ? 1 : items.book.val ? 4 : 0;
    },
    7: function () {
      // King's Tomb
    ...
    17: function () {
      return items.boss11.val && items.boots.val ? 1 : 4;
    }, // Lumberjack Tree
    18: function () {
      return logic.darkWorldNW() && items.mirror.val ? 1 : 0;
    }, // Graveyard Ledge
    19: function () {
      return 1;
    }, // Mushroom
    ...
    39: function () {
      // Old Man
      return logic.climbDM() ? (items.lamp.val ? 1 : 2) : 0;
    },
    40: function () {
      // Spectacle Rock Cave
      return logic.climbDM() ? (logic.DMlight() ? 1 : 2) : 0;
    },
    41: function () {
      // Ether Tablet
    ...
    43: function () {
      // Spiral Cave
      return logic.eastDM() ? (logic.DMlight() ? 1 : 2) : 0;
    },
    44: function () {
      // Mimic Cave
  ...
  dungeons: {
      ...
      if (settings.keyMode == 1) {
          ...
          max = entry
            ? 1 + //map chest
              (maxKey >= 1 || glove
                ...
                : boots
                ? 1
                : 0) // with no glove, best case is that torch has item
            : 0;
        }
      } else {
        // REGULAR LOGIC
        ...
        min = entry
          ? fightLanmo && boots
            ...
            : boots
            ? 1
            : 0 // if key or big key on torch, might not get any items
          : 0;
    ...
    9: function () {
      ...
      if (settings.keyMode == 1) {
        ...
        min = entry && light && firerod && medallion == 1 ? 1 + (safety ? 3 : 0) + (safety && icerod ? 1 : 0) : 0;
      }
    ...
    11: function () {
      ...
      var entry = logic.entry11(),
        ...
        key = items.key11.val;
      if (settings.keyMode == 1) {
        boss = entry && sword && key == 2 ? (light ? 1 : 2) : 0;
        ...
        if (items.keyShopFound.val) {
          ...
          boss = entry && sword ? (light ? 1 : 2) : 0;

          min = entry ? (light ? 2 : 1) : 0;
          ...
          min = entry ? (minKey >= 1 && light ? 2 : 1) : 0;
        }
      } else {
        boss = entry && sword ? (light ? 1 : 2) : 0;
  ...
  keyShops: {
    ...
    4: function () {
      return logic.darkEastDM() ? (logic.DMlight() ? 1 : 2) : 0;
    }, //Dark EDM
    5: function () {
      return logic.darkWorldNW() && items.hammer.val ? 1 : 0;
    }, //Outcasts
    6: function () {
      return logic.darkWorldNW() ? 1 : 0;
    }, //DW Forest
    ...
    8: function () {
      return logic.darkWorldEast() ? 1 : 0;
    }, //DW Potion Shop
  },
  keyShopCheck: function () {
    //function for checking and applying the status of key shop access in Retro mode
    var count = 0;
    var found = false;
    ...
    $.each(logic.keyShops, function (id, test) {
      if (test()) {
        count++;
      } //if the keyShop is accessible, add it to the count

      //toggle accessibility status on map
      $('#keyShop' + id).toggleClass('unavail', test() == 0);
      $('#keyShop' + id).toggleClass('dark', test() == 2);

      if (keyShops[id].active == true) {
        found = true;
      } //check if any shop has been clicked
    });

    if (count >= 5) {
      found = true;
    } //if more than 5 shops are accessible, one must be a key shop
