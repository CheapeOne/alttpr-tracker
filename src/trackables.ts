...
export let chests = {
  ...
  6: { world: 'LW', amount: 1, xPos: 4.5, yPos: 3.0, opened: false, status: null, name: 'Master Sword Pedestal' },
  ...
  10: { world: 'LW', amount: 5, xPos: 3.5, yPos: 42.5, opened: false, status: null, name: 'Kakariko Well' },
  ...
  13: { world: 'LW', amount: 1, xPos: 9.1, yPos: 46.4, opened: false, status: null, name: 'Bottle Merchant' },
  ...
  17: { world: 'LW', amount: 1, xPos: 30.1, yPos: 7.4, opened: false, status: null, name: 'Lumberjack Tree' },
  18: { world: 'LW', amount: 1, xPos: 56.5, yPos: 27.0, opened: false, status: null, name: 'Graveyard Ledge' },
  19: { world: 'LW', amount: 1, xPos: 12.3, yPos: 8.6, opened: false, status: null, name: 'Mushroom' },
  ...
  30: { world: 'LW', amount: 1, xPos: 2.9, yPos: 70.0, opened: false, status: null, name: 'Maze Race' },
  31: { world: 'LW', amount: 1, xPos: 2.9, yPos: 91.5, opened: false, status: null, name: 'Desert Ledge' },
  ...
  41: { world: 'LW', amount: 1, xPos: 42.1, yPos: 2.6, opened: false, status: null, name: 'Ether Tablet' },
  42: { world: 'LW', amount: 1, xPos: 50.8, yPos: 8.5, opened: false, status: null, name: 'Spectacle Rock' },
  43: { world: 'LW', amount: 1, xPos: 79.6, yPos: 8.9, opened: false, status: null, name: 'Spiral Cave' },
  44: { world: 'LW', amount: 1, xPos: 84.7, yPos: 8.9, opened: false, status: null, name: 'Mimic Cave' },
  45: { world: 'LW', amount: 7, xPos: 86.0, yPos: 18.0, opened: false, status: null, name: 'Paradox Cave' },
  46: { world: 'LW', amount: 1, xPos: 81.3, yPos: 2.6, opened: false, status: null, name: 'Floating Island' },
  47: { world: 'DW', amount: 2, xPos: 84.3, yPos: 14.4, opened: false, status: null, name: 'Superbunny Cave' },
  48: { world: 'DW', amount: 3, xPos: 81.8, yPos: 4.4, opened: false, status: null, name: 'Hookshot Cave' },
  49: {
    world: 'DW',
    amount: 1,
    xPos: 84.3,
  ...
  56: { world: 'DW', amount: 1, xPos: 5.0, yPos: 46.2, opened: false, status: null, name: 'Chest Game' },
  ...
  63: { world: 'DW', amount: 1, xPos: 5.0, yPos: 69.9, opened: false, status: null, name: 'Digging Game' },
...
export let keyShops = {
  ...
  4: { world: 'DW', xPos: 85.0, yPos: 9.2, active: false, name: 'Dark Death Mountain Shop' },
  ...
  7: { world: 'DW', xPos: 32.1, yPos: 3.4, active: false, name: 'Dark World Lumberjack Shop' },
...
export const map = {
  ...
  placeMiniChests: function () {
    //puts the mini chests pips onto each dungeon on the map
    ...
    $.each(dungeons, function (id, dungeon) {
      ...
      for (let chest = 1; chest <= count; chest++) {
        ...
        //places the pip
        TweenMax.set('#chestPip' + id + '-' + chest, {
          'transform-origin': '4px -' + (dist - 5) + 'px',
          y: dist,
          rotationZ: angle,
          force3D: true,
        });
...
export const toggle = {
  ...
  boss: function (id) {
    //toggles a dungeon's completion and marks its boss icon appropriately
    dungeons[id].completed = !dungeons[id].completed;
    items['boss' + id].val = items['boss' + id].val ? 0 : 1;
    $('#boss' + id)
      .attr('class', function (i, c) {
        return c.replace(/(^|\s)state\S+/g, '');
      })
      .addClass('state' + (dungeons[id].completed ? 1 : 0));
    $('#bigPrize' + id).toggleClass('complete', dungeons[id].completed);
    logic.apply();
  },
  dungeonChest: function (id, reverse = false) {
    //increments or decrements a dungeon's open chest count
    dungeons[id].openChests += reverse ? -1 : 1;
    if (dungeons[id].openChests > dungeons[id]['chests' + settings.keyMode]) {
      dungeons[id].openChests = 0;
    }
    if (dungeons[id].openChests < 0) {
      dungeons[id].openChests = dungeons[id]['chests' + settings.keyMode];
    }
    logic.apply();
  },
  keyShop: function (id) {
    //toggles whether a shop is marked as having a key
    keyShops[id].active = !keyShops[id].active;
    $('#keyShop' + id).toggleClass('active', keyShops[id].active);
    logic.apply();
  },
  icon: function (icon, reverse = false) {
    //toggles icons on the tracker
    ...
    if (icon.id == 'mushroompowder' || icon.id == 'shovelflute') {
      //this ensures that the separated mushroom/powder and shovel/flute icons always match the combined versions of those icons
      ...
      $('#mushroom')
        ...
        .addClass('state' + items.mushroom.val);
      $('#powder')
        .attr('class', function (i, c) {
          return c.replace(/(^|\s)state\S+/g, '');
        })
        .addClass('state' + items.powder.val);
      $('#shovel')
        .attr('class', function (i, c) {
          return c.replace(/(^|\s)state\S+/g, '');
        })
        .addClass('state' + items.shovel.val);
      $('#flute')
        .attr('class', function (i, c) {
          return c.replace(/(^|\s)state\S+/g, '');
        })
        .addClass('state' + items.flute.val);
    }

    if (icon.id == 'mushroom' || icon.id == 'powder' || icon.id == 'flute' || icon.id == 'shovel') {
      //and vice versa, this ensures that the combined mushroom/powder and shovel/flute icons always match the separated versions of those icons
      ...
      $('#mushroompowder')
        ...
        .addClass('state' + items.mushroompowder.val);
      $('#shovelflute')
        .attr('class', function (i, c) {
          return c.replace(/(^|\s)state\S+/g, '');
        })
        .addClass('state' + items.shovelflute.val);
    }
